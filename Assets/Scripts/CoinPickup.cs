...
public class CoinPickup : MonoBehaviour
{
    ...
    [SerializeField] private GameObject player; // The variable to keep track of the player. GK
    ...
    void Start()
    {
        ...
        this.playerController = player.GetComponent<BasePlayerController>();
    }
    ...
    void Update()
    {
        ...
        {
            transform.position = Vector3.MoveTowards(transform.position, playerController.transform.position, moveSpeed * Time.deltaTime); // Move the experience towards the player. GK
        }
        else
        {
            ...
            if (checkCounter <= 0f) // If the check counter is less than or equal to 0. GK
            {
                ...
                if (Vector3.Distance(transform.position, playerController.transform.position) < playerController.pickupRange) // If the distance between the experience and the player is less than the pickup range. GK
                {
                    movingToPlayer = true; // Set movingToPlayer to true. GK
                    moveSpeed += playerController.moveSpeed; // Add the player's move speed to the experience's move speed. GK
                }
            }
        }
    }
