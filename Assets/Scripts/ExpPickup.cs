...
public class ExpPickup : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            transform.position = Vector3.MoveTowards(transform.position, player.transform.position, moveSpeed * Time.deltaTime); // Move the experience towards the player. GK
        }
        else
        {
            ...
            if (checkCounter <= 0f) // If the check counter is less than or equal to 0. GK
            {
                ...
                if (Vector3.Distance(transform.position, player.transform.position) < playerController.pickupRange) // If the distance between the experience and the player is less than the pickup range. GK
                {
